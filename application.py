...
def register():
    ...
    # if POST, validate and commit to database
        ...
        if not request.form.get("first_name"):
            return render_template("error.html", message="Debes proporcionar un nombre")
        elif not request.form.get("last_name"):
            return render_template("error.html", message="Debes proporcionar un apellido")
        elif  not request.form.get("email"):
            return render_template("error.html", message="Debes proporcionar un email")
        elif not request.form.get("password1") or not request.form.get("password2"):
            return render_template("error.html", message="Debes proporcionar una contraseña")
...
def login():
    ...
    if request.method == "POST":
        ...
        # User exists ?
        if Q is None:
            return render_template("error.html", message="El usuario no existe")
        # Valid password ?
        if not check_password_hash( Q.password, form_password):
            return  render_template("error.html", message = "Contraseña Incorrecta")
...
def search():
    if request.method == "GET":
        ...
        if query is None:
            return render_template("error.html", message="El campo de busqueda no puede estar vacio!")
        try:
            ...
            return render_template("error.html", message=e)
        if not result:
            return render_template("error.html", message="El dato ingresado no existe")
...
def details(bookid):
    if request.method == "GET":
        ...
        try:
            ...
            goodreads = requests.get(url, params={"key": key, "isbns": result.isbn})
        ...
        if not result:
            return render_template("error.html", message="id de libro invalido")
        ...
        if user_reviewed_before:
            return render_template("error.html", message = "Ya has proporcionado reseña anteriormente!")
        ######## Proceed to get user comment ###########
        user_comment = request.form.get("comments")
        user_rating = request.form.get("rating")

        if not user_comment:
            return render_template("error.html", message="El campo de comentario no puede estar vacio")
